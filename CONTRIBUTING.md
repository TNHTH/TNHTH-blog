...
`main` 合并后由 CI 验证。仓库连接 Vercel，`main` 更新后触发生产部署；构建环境只读取仓库内公开快照，不访问私人 vault。

撤回文章时，先在分支中删除或恢复对应公开快照，再运行完整交付门并提交 PR。生产故障优先在 Vercel Dashboard 回滚到上一个成功部署，其次回滚 Git 提交后重新验证。发布失败时保留失败日志，修复根因后重新运行相同检查，不要跳过安全扫描。
...
禁止把私人 vault 根路径或其他本地绝对路径写入公开内容或构建产物。临时截图、日志和审计中间文件放在项目临时目录，确认不需要后再按本地工作区规则处理。
